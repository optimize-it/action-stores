...
def create_or_update_disks(params: DiskCreationParameters) -> Union[DiskResponseModel, dict]:
    try:
        ...
        endpoint = f"/resourceGroups/{resourceGroupName}/providers/Microsoft.Compute/disks/{diskName}"
        api_version = "2021-12-01"

        response_data = put_wrapper(endpoint, api_version, data=disk_data)
...
def listall_azure_disks(params: DiskListParameters) -> List[DiskListModel]:
    try:
        #subscriptionId = params.subscriptionId
        endpoint = f"/providers/Microsoft.Compute/disks"
        api_version = "2021-12-01"
        response_data = get_wrapper(endpoint, api_version)
...
def list_by_rg_azure_disks(params: DiskListByRGParameters) -> List[DiskListModel]:
    try:
        #subscriptionId = params.subscriptionId
        endpoint = f"/resourceGroups/{params.resourceGroupName}/providers/Microsoft.Compute/disks"
        api_version = "2021-12-01"
        response_data = get_wrapper(endpoint, api_version)
        disk_list = response_data.get('value', [])
        return [DiskListModel(**disk) for disk in disk_list]
...
def delete_azure_disks(params: DiskDeleteParameters):
    try:
        #subscriptionId = params.subscriptionId
        endpoint = f"/resourceGroups/{params.resourceGroupName}/providers/Microsoft.Compute/disks/{params.diskName}"
        api_version = "2021-12-01"
